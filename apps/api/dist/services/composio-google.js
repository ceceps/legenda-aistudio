...
export async function uploadFileToDriveViaComposio(fileContent, fileName, mimeType, folderId) {
...
async function makeFilePublicViaComposio(fileId) {
...
export async function synthesizeVoiceOverViaComposio(text, languageCode = 'id-ID', voiceName = 'id-ID-Wavenet-B') {
...
export function isComposioGoogleConfigured() {
    return Boolean(COMPOSIO_API_KEY && COMPOSIO_ENTITY_ID);
}
// ── Helper: Get Composio connection status ───────────────────────────────────
export async function getComposioGoogleConnectionStatus() {
    try {
        const res = await fetch(`https://api.composio.dev/api/v1/connectedAccounts?entityId=${COMPOSIO_ENTITY_ID}`, {
...
//# sourceMappingURL=composio-google.js.map