...
export const assembleWorker = new Worker('assemble', async (job) => {
    ...
    if (voAsset) {
        ...
        const narrationText = screenplay?.scenes?.map((s, i) => `Scene ${i + 1}: ${s.voiceOver}`).join('\n\n') ?? '';
        if (narrationText) {
    ...
    if (backsoundPath)
        mergeOptions.backsoundPath = backsoundPath;
    if (soundtrackPath)
        mergeOptions.soundtrackPath = soundtrackPath;
    if (voiceOverPath)
        mergeOptions.voiceOverPath = voiceOverPath;
...
//# sourceMappingURL=assemble.worker.js.map