...
audioRouter.get('/:id/audio/:audioId', async (req, res) => {
...
audioRouter.post('/:id/audio/:audioId/regenerate', async (req, res) => {
    ...
    try {
        ...
        if (audio.type === AudioType.VOICE_OVER) {
            ...
            const { shareLink } = await uploadFileToDrive(tmpPath, `vo-${audioId}.mp3`, 'audio/mpeg');
            audioUrl = shareLink;
            unlinkSync(tmpPath);
        }
...
audioRouter.delete('/:id/audio/:audioId', async (req, res) => {
    const { audioId } = req.params;
    const audio = await prisma.audioAsset.findUnique({ where: { id: audioId } });
    if (!audio) {
        res.status(404).json({ success: false, error: 'Audio asset not found' });
...
audioRouter.post('/:id/audio/batch-regenerate', async (req, res) => {
    ...
    const results = await Promise.allSettled(audioAssets.map(async (audio) => {
        try {
            let audioUrl;
            if (audio.type === AudioType.VOICE_OVER) {
                const tmpPath = `/tmp/${audio.id}-vo.mp3`;
                await synthesizeVoiceOver(audio.sunoPrompt, tmpPath);
                const { shareLink } = await uploadFileToDrive(tmpPath, `vo-${audio.id}.mp3`, 'audio/mpeg');
                audioUrl = shareLink;
                unlinkSync(tmpPath);
            }
            else if (audio.type === AudioType.SOUNDTRACK) {
