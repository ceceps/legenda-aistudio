...
import {
  uploadFileToDriveViaComposio,
  synthesizeVoiceOverViaComposio,
  isComposioGoogleConfigured,
  getComposioGoogleConnectionStatus,
} from './composio-google.js';
import { readFileSync, type PathLike } from 'fs';

// ── Configuration Detection ───────────────────────────────────────────────────

function hasDirectGoogleCredentials(): boolean {
  return Boolean(process.env.GOOGLE_SERVICE_ACCOUNT_JSON);
}

function hasComposioGoogleAccess(): boolean {
  return isComposioGoogleConfigured();
}

export enum GoogleServiceMode {
  DIRECT = 'direct',
  COMPOSIO = 'composio',
  NONE = 'none',
}

export function getGoogleServiceMode(): GoogleServiceMode {
  if (hasDirectGoogleCredentials()) return GoogleServiceMode.DIRECT;
  if (hasComposioGoogleAccess()) return GoogleServiceMode.COMPOSIO;
...
export async function uploadFileToDrive(
  ...
  switch (mode) {
    ...
    case GoogleServiceMode.COMPOSIO:
      ...
      return uploadFileToDriveViaComposio(
        fileContent,
        fileName,
        mimeType,
    ...
    case GoogleServiceMode.NONE:
      throw new Error(
        'No Google Drive access configured. Set GOOGLE_SERVICE_ACCOUNT_JSON or configure Composio Google integration.',
      );
  }
}
...
export async function synthesizeVoiceOver(
  ...
  switch (mode) {
    ...
    case GoogleServiceMode.NONE:
      throw new Error(
        'No Google TTS access configured. Set GOOGLE_SERVICE_ACCOUNT_JSON or configure Composio Google integration.',
      );
  }
}
...
export async function checkGoogleServicesHealth(): Promise<{
