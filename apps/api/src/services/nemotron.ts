...
import { type Screenplay, type AssetPrompt, AssetType } from '@legenda/shared-types';
...
export async function generateAssetPrompts(screenplay: Screenplay): Promise<AssetPrompt[]> {
...
[{
  ...
  if (!match) throw new Error('Nemotron: no JSON array in response');
  return JSON.parse(match[0]) as AssetPrompt[];
}
