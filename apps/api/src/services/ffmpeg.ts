...
export function getTmpPath(projectId: string, filename: string): string {
...
export interface MergeOptions {
  projectId: string;
  clipPaths: string[];       // ordered list of video clip file paths
  backsoundPath?: string;    // background music (low volume)
  soundtrackPath?: string;   // main song
  voiceOverPath?: string;    // narrator audio (foreground)
  outputPath: string;
}

export async function mergeVideoWithAudio(opts: MergeOptions): Promise<string> {
  ensureTmpDir();
  ...
  const inputArgs: string[] = ['-i', mergedVideoPath];
