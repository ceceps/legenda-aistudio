...
export const assembleWorker = new Worker<PipelineJobData>(
  ...
  async (job) => {
    ...
    broadcastProgress({
      projectId, stage: ProjectStatus.ASSEMBLING, progress: 95,
      message: 'Mengunduh clip & audio, menyiapkan assembly...', timestamp: new Date().toISOString(),
    });
    ...
    broadcastProgress({
      projectId, stage: ProjectStatus.ASSEMBLING, progress: 96,
      message: 'Mengunduh video clip...', timestamp: new Date().toISOString(),
    });
    ...
    const backsound = audioAssets.find((a) => a.type === AudioType.BACKSOUND && a.audioUrl);
    const soundtrack = audioAssets.find((a) => a.type === AudioType.SOUNDTRACK && a.audioUrl);
    ...
    if (voAsset) {
      broadcastProgress({
        projectId, stage: ProjectStatus.ASSEMBLING, progress: 97,
        message: 'Membuat voice over narator...', timestamp: new Date().toISOString(),
      });
      const screenplay = project.screenplay as { scenes?: { voiceOver: string }[] } | null;
      const narrationText = screenplay?.scenes
        ?.map((s, i) => `Scene ${i + 1}: ${s.voiceOver}`)
        .join('\n\n') ?? '';
    ...
    // ── FFmpeg merge ──────────────────────────────────────────────────────────
    broadcastProgress({
      projectId, stage: ProjectStatus.ASSEMBLING, progress: 98,
      message: 'FFmpeg menggabungkan video & audio...', timestamp: new Date().toISOString(),
    });
    ...
    broadcastProgress({
      projectId, stage: ProjectStatus.ASSEMBLING, progress: 99,
      message: 'Mengunggah video ke Google Drive...', timestamp: new Date().toISOString(),
    });
    ...
    broadcastProgress({
      projectId, stage: ProjectStatus.COMPLETED, progress: 100,
...
assembleWorker.on('failed', async (job, err) => {
  ...
  broadcastProgress({
    ...
    message: `Assembly gagal: ${err.message}`, timestamp: new Date().toISOString(),
  });
});
